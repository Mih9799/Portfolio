...
public class TrainerBotAI : MonoBehaviour
{
    ...
    void Update()
    {
        if (dead == false && activated == true)
        {
            ...
            if (m_player != null && stunned == false)
            {
                ...
                Quaternion.LookRotation(direction) * Quaternion.Euler(270, 90, 0), 0.75f);
                if (IsRightD == true)
                {
                    Orientation.transform.rotation = Quaternion.Slerp(Orientation.transform.rotation, Quaternion.Euler(-90, 0, 180), 25f * Time.deltaTime);
                }
                else
                {
                    Orientation.transform.rotation = Quaternion.Slerp(Orientation.transform.rotation, Quaternion.Euler(-90, 0, 0), 25f * Time.deltaTime);
                }
            }
            else
            {
